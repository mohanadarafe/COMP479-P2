...
The following block executes the first block Naive Indexer. In order to run the indexer, execute the following command
    ...
    MODULE 1 EXECUTION
    python indexer_builder.py --path "<path to data>" -o "output/<output_file_name>.json"
    python indexer_builder.py --path "./data" -o "output/unsorted_data.json"
    
    <path_to_data>: path to reuters data files
    <output_file_name>: name your file to see output
'''
import asserts, indexer, nltk, time
...
'''
start = time.time()
for reuters_file_content in tqdm(indexer.module1(path)):
