...
The following block executes the first block Naive Indexer. In order to run the indexer, execute the following command
    ...
    python indexer_sorter.py -i "output/unsorted_data.json" -o "sorted_data.json"
...
'''
start = time.time()
for reuters_file_content in tqdm(indexer.module2((json.loads(line) for line in args.input_file))):
